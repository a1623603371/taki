...
public class ResponseResult<T> implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        if (returnType.getGenericParameterType().equals(String.class)){
            ...
            try {
                return objectMapper.writeValueAsString(com.taki.utis.ResponseData.success(body));
            } catch (JsonProcessingException e) {
                // 加入自定义的统一异常处理
                e.printStackTrace();
            }
        }else if (body instanceof ExceptionResult){ // 判断是否是异常对象类型
            com.taki.utis.ResponseData.error(CodeEnum.SYSTEM_ERROR,body);
        }

        return com.taki.utis.ResponseData.success(body);
    }

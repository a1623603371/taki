package com.taki.consistency.config;

import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * @ClassName TendConsistencyConfiguration
 * @Description 框架级配置参数
 * @Author Long
 * @Date 2022/8/31 21:52
 * @Version 1.0
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class TendConsistencyConfiguration {

    /**
     *  线程池核心数量
     */
    public Integer threadCorePoolSize;

    /**
     *  线程池最大线程数量
     */
    public  Integer threadMaxPoolSize;

    /**
     * 调度型任务线程池的队列大小
     */
    public Integer threadPoolQueueSize;

    /**
     *线程池中无任务时线程存活时间
     */
    public Long threadPoolKeepAliveTime;

    /**
     * 可选值：【SECONDS,MINUTES,HOURS,DAYS,NANOSECONDS,MICROSECONDS,MILLISECONDS】线程池中无任务时线程池存活时间单位
     */
    public  String threadPoolKeepAliveUnit;


    /**
     * 触发降级逻辑问题 任务执行次数 如果
     */
    public  Integer failCountThreshold;

    /**
     *  任务表是否进行分表
     */
    public  Boolean taskSharded = false;

    /**
     * 这里要配置类型全路径且类要实现com.taki.consistency.query.TaskTimeRangQuery 如：com.xxx.TaskTimeLineQuery
     */
    private String taskScheduleTimeRangeClassName = "";

    /**
     *生成任务表分片key的ClassName 这里要配置类型全路径且类要实现 com.taki.consistency.custom.shard.ShardingKeyGenerator接口
     */
    private String shardingKeyGeneratorClassName="";

}
